...
String oledStatusLine1 = "Booting";
String oledStatusLine2 = "";
...
void refreshOled() {
  ...
  display.println("ST3020 test");
  display.print("ID: ");
  display.println(selectedServoId);
  display.println(oledStatusLine1);
  display.println(oledStatusLine2);
  display.display();
}

void setOledStatus(const String& line1, const String& line2 = "") {
  oledStatusLine1 = line1;
  oledStatusLine2 = line2;
  refreshOled();
}
...
void beginOled() {
  ...
  setOledStatus("UART ready", "Waiting cmds");
}
...
void printStatusHeader() {
  ...
  Serial.print("OLED: ");
  Serial.println(oledAvailable ? "ok" : "missing");
}
...
bool readFeedback(uint8_t id) {
  if (st.FeedBack(id) == -1) {
    ...
    Serial.println(id);
    setOledStatus("Feedback error", "ID " + String(id));
    return false;
  }
  return true;
}
...
void printFeedback(uint8_t id) {
  ...
  setOledStatus("Pos " + String(pos), "V " + String(voltage / 10.0f, 1) + " T " +
...
void pingServo(uint8_t id) {
  ...
  if (pingResult < 0) {
    ...
    Serial.println(id);
    setOledStatus("No response", "ID " + String(id));
    return;
  }
  ...
  setOledStatus("Ping OK ID " + String(id), model >= 0 ? "Model " + String(model) : "Model ?");
}
...
void setTorque(uint8_t id, bool enabled) {
  ...
  Serial.print(" for ID ");
  Serial.println(id);
  setOledStatus(enabled ? "Torque ON" : "Torque OFF", "ID " + String(id));
}
...
void stopServo(uint8_t id) {
  ...
  Serial.println(id);
  setOledStatus("Stop", "ID " + String(id));
}
...
void moveServo(uint8_t id, int32_t position, uint16_t speed, uint8_t acc) {
  ...
  Serial.print(", acc=");
  Serial.println(acc);
  setOledStatus("Move ID " + String(id), "P" + String(position) + " S" + String(speed));
}
...
void scanServoIds() {
  ...
  if (!foundAny) {
    ...
    setOledStatus("Scan done", "No servo");
  } else {
    setOledStatus("Scan done", "Found servo(s)");
  }
}
...
void setServoId(uint8_t oldId, uint8_t newId) {
  ...
  if (pingResult < 0) {
    ...
    setOledStatus("No old ID", String(oldId));
    return;
  }
  ...
  if (st.Ping(newId) >= 0) {
    ...
    Serial.println(newId);
    setOledStatus("ID changed", String(oldId) + " -> " + String(newId));
  } else {
    Serial.println("Warning: new ID not confirmed yet.");
    setOledStatus("ID change sent", "Confirm failed");
  }
}

void setServoMiddle(uint8_t id) {
  const int pingResult = st.Ping(id);
  if (pingResult < 0) {
    Serial.print("No response on ID ");
    Serial.println(id);
  ...
  Serial.print("Set middle requested for ID ");
  Serial.println(id);
  setOledStatus("Set middle", "ID " + String(id));
}
...
void handleCommand(const String& line) {
  if (line == "help") {
    printHelp();
    setOledStatus("Help printed");
    return;
  }

  if (line == "status") {
    printStatusHeader();
    setOledStatus("Status printed");
    return;
  }
  ...
  if (line.startsWith("id=")) {
    ...
    Serial.print("Selected servo ID: ");
    Serial.println(selectedServoId);
    setOledStatus("Selected ID", String(selectedServoId));
    return;
  }
  ...
  if (line == "setmiddle") {
    setServoMiddle(selectedServoId);
    return;
  }
  ...
  Serial.println(line);
  setOledStatus("Unknown cmd", line);
  printHelp();
}
